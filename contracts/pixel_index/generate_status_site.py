...
Adapted from pixel-agents-cogs' contracts/pixel_index/generate_status_site.py (same
result-document shape, same JSON API layout) for this repo's single contract. The
input directory contains one structured result per environment, produced by
verify.py. The output is a self-contained GitHub Pages artifact: it keeps no history
and exposes the same snapshot as accessible HTML, a versioned JSON API, and
Shields-compatible badge documents.
"""
...
def load_results(results_dir: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(results_dir.rglob("*.json")):
        ...
        if not re.fullmatch(r"[a-z0-9][a-z0-9_-]*", environment):
            raise ValueError(
                f"Contract result {path} has an unsafe environment name: {environment!r}"
            )
    ...
    return sorted(
        results.values(),
        key=lambda item: (ENVIRONMENT_PRIORITY.get(item["environment"], 100), item["environment"]),
    )
...
def build_snapshot(
    ...
    *,
    ...
    overall = min(
        (result["status"] for result in results), key=lambda value: STATUS_PRIORITY[value]
    )
...
def badge_document(label: str, status: str) -> dict[str, Any]:
...
def render_environment_cards(environments: list[dict[str, Any]]) -> str:
    cards = []
    for environment in environments:
        ...
        checked = (
            f'<time datetime="{escape(checked_at)}">{escape(checked_at)}</time>'
            if checked_at
            else "Not completed"
        )
        cards.append(
...
def check_names(environments: list[dict[str, Any]]) -> list[str]:
    names: list[str] = []
    for environment in environments:
...
def render_check_table(environments: list[dict[str, Any]]) -> str:
    names = check_names(environments)
    headers = "".join(
        f'<th scope="col">{escape(environment["environment"].title())}</th>'
        for environment in environments
    )
    rows = []
    for name in names:
    ...
    if not rows:
        rows.append(
            f'<tr><td colspan="{len(environments) + 1}" class="empty">No check results were produced.</td></tr>'
        )
    return f"""
...
def generate_site(
    ...
    *,
    ...
    for environment in results:
        ...
        write_json(
            output_dir / "api" / "v1" / "environments" / f"{name}.json", environment_document
        )
        write_json(
            output_dir / "api" / "v1" / "badges" / f"{name}.json",
            badge_document(f"pixel-index {name}", environment["status"]),
        )
