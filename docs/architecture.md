...
One owner, one workspace volume, one service process, one active script/render job. FastAPI and
the official MCP SDK share an ASGI application. SQLite stores project pointers, target
configurations, artifacts and the FIFO queue. Scene revisions store native pixel definitions.
No server-side AI model is involved.
...
requires a target configuration and a valid pixel definition. Authoring scripts remain an advanced
preparation/editing path, not a rendering fallback.

`pixel_art.py` is the stdlib-only Canvas/PixelArt helper shared with the engine subprocess.
The typed write
...
3. The engine subprocess loads the previous scene state, executes the write, validates
   canvas/pose/palette requirements and saves the result. The application validates the
   returned definition again.
...
5. Render submission captures the selected revision and current configuration. Both the engine
   and exporter require pixel art; reconfiguring a target can require rewriting the source.
...
settings. `engine/render.py` resolves exact patches for each native view/frame onto a
blank transparent canvas per view/frame, bypassing quantization entirely.
...
Application code and the script/render engine both run under the same Python 3.12 interpreter,
but never in the same process: a JSON request bridges them across a fresh background subprocess,
isolating trusted script execution from the API process. Default Compose exposes only loopback,
uses a named volume and read-only root, and mounts no host credentials or Docker socket.
...
logs and timeouts are bounded. Preview output is capped at 4194304 pixels; artifact inline
content at 1 MiB. Limits are discoverable. Readiness requires a running worker.
...
configuration snapshots and failed-edit rollback. An end-to-end test covers a scripted edit and
render round-trip through the real engine subprocess. Cold-client Docker tests use only MCP tool
discovery and inline profile examples to create, edit, render and inspect all three asset kinds,
without reading repository examples or HTTP artifacts. Python examples additionally regress
`execute_pixel_script`'s programmatic authoring path.
