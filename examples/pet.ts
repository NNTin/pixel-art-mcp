...
export default function main(scene: Scene): void {
  const art = new PixelArt(
    { D: "#453c42", S: "#ad7846", W: "#dca354", H: "#f2d38c", N: "#292f39", R: "#c75b62" },
    { 0: [16, 32], 180: [16, 32], 90: [32, 32] },
  );
  for (const angle of [0, 180, 90] as const) {
    for (let frame = 1; frame <= 6; frame++) {
      const phase = [0, -1, 1, 0, -1, 1][frame - 1];
      const canvas = new Canvas(angle === 90 ? 32 : 16, 32);
      if (angle === 90) {
        canvas.stamp(6, 18, [
          "..WWWWWWWWWW....",
          ".WHHHHHHHHHWW...",
        ...
        canvas.rect(4, 26 + Math.min(phase, 0), 3, 3, "W");
        canvas.rect(9, 26 - Math.max(phase, 0), 3, 3, "W");
        canvas.rect(4, 29 + Math.min(phase, 0), 3, 1, "D");
        canvas.rect(9, 29 - Math.max(phase, 0), 3, 1, "D");
        if (angle === 0) {
          canvas.stamp(2, 13, [
            "...WWWWWW...",
            "..WHHHHHHW..",
      ...
      art.layer("dog", angle, canvas, { frame, min_pixels: 110 });
    }
  }
  art.save(scene);
}
